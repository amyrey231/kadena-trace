...
interface CyberNodeData {
  ...
  txCount?: number;
  firstSeen?: string;
  totalFound?: number; // Added to support real backend counts
  [key: string]: unknown;
}
...
export default function FraudTracer() {
          ...
          <div className="absolute top-6 left-6 z-10 w-72 bg-white/95 dark:bg-[#080B10]/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-2xl flex flex-col gap-6 transition-colors duration-500 animate-in fade-in slide-in-from-left-8">
            ...
            <div className="space-y-4">
               <div className="flex justify-between items-center text-xs font-mono border-b border-slate-100 dark:border-slate-800 pb-3 transition-colors duration-500">
                 <span className="text-slate-500">Connected Wallets</span>
                 <span className="text-slate-800 dark:text-white font-bold">{nodes.length > 0 ? ((nodes[0].data as CyberNodeData).totalFound || nodes.length - 1) : 0} Found</span>
               </div>
               <div className="flex justify-between items-center text-xs font-mono border-b border-slate-100 dark:border-slate-800 pb-3 transition-colors duration-500">
    ...
    <div className="h-screen w-full flex flex-col bg-slate-50 dark:bg-[#080B10] text-slate-900 dark:text-slate-100 overflow-hidden">
      ...
      <header className="flex flex-col px-4 py-3 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-[#080B10]/95 backdrop-blur-xl z-30 gap-3 shadow-sm">
        ...
        <div className="w-full relative">
          ...
          <input 
            ...
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)} // Fixed e.target.value
            onKeyDown={(e) => e.key === 'Enter' && handleAnalyze()}
          />
          <button 
            onClick={handleAnalyze}
      ...
      <main className="flex-1 relative flex flex-col">
        ...
        {nodes.length > 0 && !selectedNode && (
          <div className="absolute bottom-6 left-4 right-4 z-20 bg-white/95 dark:bg-[#080B10]/95 backdrop-blur-2xl border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-[0_10px_40px_rgba(0,0,0,0.15)] flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div>
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-3 flex items-center gap-2">
                <Activity className="w-3 h-3 text-rose-500" /> Case Overview
              </h2>
              <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Stolen Funds Tracked</p>
                <h3 className="text-xl font-black font-mono text-rose-600 dark:text-rose-400 leading-none">
                  {nodes.reduce((acc, node) => acc + parseFloat((node.data as CyberNodeData).volume || "0"), 0).toFixed(4)}
  ...
  {searchQuery.startsWith('k:') ? 'Kadena Native' : nodes.length > 3 ? 'Binance Bridge' : 'Ethereum Mainnet'}
               ...
               <div className="flex justify-between items-center text-[10px] font-mono">
                 <span className="text-slate-500">Pact Anchoring</span>
                 <span className="text-emerald-600 dark:text-emerald-500 font-bold flex items-center gap-1"><Lock size={10}/> Verified</span>
               </div>
            </div>
            ...
            <button 
              onClick={handleAnchor}
              disabled={isAnchoring || isAnchored}
              className={`w-full py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-md flex items-center justify-center gap-2 ${
                isAnchored ? 'bg-emerald-600 text-white' : 'bg-slate-900 dark:bg-slate-800 text-white'
              }`}
            >
              {isAnchoring ? (
                ...
                <><FileText className="w-4 h-4" /> Register on Kadena</>
              )}
            </button>
          </div>
        )}
